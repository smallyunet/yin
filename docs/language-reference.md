...
[Yin 0.16 language specification](language-specification.md). Files in
...
```bash
java -jar yin-0.16.0.jar examples/cli/parse-values.yin 10 bad 32
```
...
```bash
java -jar yin-0.16.0.jar --json program.yin < request.json
```
...
```bash
java -jar yin-0.16.0.jar --capabilities program.yin
```
