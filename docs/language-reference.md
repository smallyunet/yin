...
[Yin 0.17 language specification](language-specification.md). Files in
...
```bash
java -jar yin-0.17.0.jar examples/cli/parse-values.yin 10 bad 32
```
...
```bash
java -jar yin-0.17.0.jar --json program.yin < request.json
```
...
```bash
java -jar yin-0.17.0.jar --capabilities program.yin
```
...
- there is no module system, package manager, or general-purpose native compiler
