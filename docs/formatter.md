...
```bash
java -jar target/yin-0.11.0.jar --format program.yin
```
...
```bash
java -jar target/yin-0.11.0.jar --format --check tests/*.yin
```

Rewrite one or more files in place:

```bash
java -jar target/yin-0.11.0.jar --format --write program.yin
```
