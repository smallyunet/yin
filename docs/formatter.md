...
```bash
java -jar target/yin-0.6.0-SNAPSHOT.jar --format program.yin
```
...
```bash
java -jar target/yin-0.6.0-SNAPSHOT.jar --format --check tests/*.yin
```

Rewrite one or more files in place:

```bash
java -jar target/yin-0.6.0-SNAPSHOT.jar --format --write program.yin
```
